...
"""Verify immutable InstantTensor wheel release assets before publication or promotion."""
...
def verify_release(
    ...
    *,
    ...
    require(beta_tag == f"instanttensor-cu134-beta-{source_commit}", "invalid beta tag")
    ...
    require(
        ...
        "release must contain exactly one instanttensor package",
    )
    package = packages[0]
    require(
        package.get("name") == "instanttensor", "release package must be instanttensor"
    )
    wheel_name = package.get("file", "")
    require(
        isinstance(wheel_name, str)
        and wheel_name.startswith("instanttensor-")
        and wheel_name.endswith(".whl")
        and all(c.isascii() and (c.isalnum() or c in "_.+-") for c in wheel_name),
        "invalid wheel filename",
    )
    archive = f"instanttensor-cu134-{source_commit}.tar.zst"
    ...
    if promotion:
        ...
        required_fields = {
            "schema": "local-inference-instanttensor-promotion/v1",
